...
def create_knowledge_graph(data):
    ...
    for row in data:
        try:
            ...
            if not existing_job:
                job_node = Node("Job", title=job_title, salary_range=row[3], responsibilities=row[5])
            ...
            if education_requirement_name in education_requirement_dict.keys():
                ...
                relationship = Relationship(job_node, "REQUIRES", education_requirement_node)
                graph.merge(relationship)
            ...
            # 建立关系
            relationship1 = Relationship(company_node, "RECRUITS", job_node)
            relationship2 = Relationship(company_node, "HAS_CONTACT", contact_node)
            relationship3 = Relationship(company_node, "HAS_ACTIVITY", activity_node)
            relationship4 = Relationship(company_node, "HAS_ADDRESS", address_node)
            relationship5 = Relationship(company_node, "HAS_WEBSITE", website_node)
