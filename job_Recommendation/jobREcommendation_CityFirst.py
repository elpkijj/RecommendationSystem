...
def recommend_jobs(resume_data_path, all_info_path, work_embeddings_path, top_n=30):
    ...
    # 计算简历向量与每个岗位向量之间的内积
    scores = {work_id: np.dot(resume_embedding, work_embedding) for work_id, work_embedding in work_embeddings.items()}

    # 使用normalize_scores函数归一化评分
    normalized_scores = normalize_scores(scores)
    i=0
    # 推荐逻辑
    for work_id, normalized_score in sorted(normalized_scores.items(), key=lambda item: item[1], reverse=True)[:top_n]:
        ...
        if work_address!=resume_city or work_address == "Unknown" or work_salary == "Unknown" or work_education == "Unknown":
            continue
