...
def recommend_jobs(resume_data_path, all_info_path, work_embeddings_path, top_n=30):
    ...
    for work_id, work_embedding in work_embeddings.items():
        ...
        min_work_salary, max_work_salary = parse_salary(work_salary)
        pre_salary_scores[work_id] = min_work_salary + max_work_salary

    # 使用normalize_scores函数归一化评分
    normalized_scores = normalize_scores(scores)
    i=0
    # 推荐逻辑
    for work_id, edu_score in sorted(pre_salary_scores.items(), key=lambda item: item[1], reverse=True)[:top_n]:
